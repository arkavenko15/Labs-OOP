...
public:
	...
	friend std::ostream& operator<< (std::ostream &out, const CBox &x);
	friend std::istream& operator>> (std::istream &input,  CBox &x);
...
std::ostream & operator<<(std::ostream & out, const CBox & x)
{
	out << "Point(" << x.m_dblHeight << ", " << x.m_dblLength << ", " << x.m_dblW³dth << ")";
	return out;
}
std::istream & operator >>(std::istream & input, CBox & x)
{
	cout << "Enter:" << endl;
	input >> x.m_dblHeight; 
	input >> x.m_dblHeight; 
...
int main() {
	...
	c = a; 
	c.Print();
	cout << "Demonstration of the operator of the addition (<<)" << endl;
	std::cout << a;
	std::cout << b;
	std::cout << c;
	cout << endl;
